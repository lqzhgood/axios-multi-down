...
function E(t, r = A) {
  return t.down = async function(e, n, s) {
    ...
    const a = $(c), [d, u] = await B(t, a, o);
    if (!d || !u)
      return await N(t, o, a);
    {
      const l = x(u, a.blockSize);
      a.max = a.max <= l.length ? a.max : l.length, m(a, "preDown", l, a);
      let i;
...
function N(t, r, e) {
  ...
  return new Promise((c, a) => {
    function d() {
      return t(r).then((u) => (n.isDown = !0, n.e = u.headers["content-length"] - 1, n.resp = u, m(e, "data", n, s, e), m(e, "end", s, e), o = { ...u, ...o }, c(o), o)).catch((u) => (n.isDown = !1, m(e, "blockError", n, s, e), n.retryCount < e.maxRetries ? (n.retryCount++, setTimeout(() => {
        d();
      }, e.retryInterval)) : (u.response && (o = { ...u.response, downResponse: o }), u.downResponse = o, e.errMode !== b.WAIT && a(u), m(
        e,
        "finishErr",
        s.filter((l) => !l.resp),
        s,
    ...
    n.forEach((i) => {
      const M = (R = !1) => new Promise((D) => {
        ...
        t({ ...r, headers: T, responseType: "arraybuffer" }).then((h) => {
          ...
          if (l === 0) {
            if (n.filter((f) => !f.resp).length !== 0) {
              e.errMode !== b.WAIT && (h.downResponse = a, c(h)), m(
                e,
                "finishErr",
                n.filter((f) => !f.resp),
                n,
